...
$url = new moodle_url('/local/sharedresources/pro/admin/view_access_table.php');
$PAGE->set_url($url);

$libadmin = get_library_admin();
$renderer = new \local_sharedresource\output\admin_renderer($PAGE, '');

echo $OUTPUT->header();

// Provide infor about all bound repos (remote)
echo $OUTPUT->heading(get_string('localaccesstable', 'local_sharedresources'));

echo $renderer->resource_access_table($libadmin->get_access_resource_infos());
